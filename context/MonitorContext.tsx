...
export const MonitorProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchQueueData = useCallback(async () => {
    try {
      ...
      const rpsTotal = rpsQueues.reduce(
        (sum, queue) => sum + queue.totalMessagesReady,
        0
      );
      ...
      setQueueData({
        ...
        nfseQueryNfse:
          data.find((queue) => queue.name === "nfse-queryNfse") || null,
        nfseQueryRpsProtocol:
          data.find((queue) => queue.name === "nfse-queryRpsProtocol") || null,
  ...
  const processData = useCallback((data: ApiResponse[]) => {
    ...
    const states = updatedStatuses
      .filter(
        (status) =>
          status.name &&
          status.name.length <= 4 &&
          status.group &&
          status.group.name === "Autorizadores de NF-e"
      )
